...
@EnableConfigurationProperties({
		LabelImageProcessorProperties.class, TensorflowProcessorProperties.class})
@Import(TensorflowProcessorConfiguration.class)
public class LabelImageProcessorConfiguration {

	private static final Log logger = LogFactory.getLog(LabelImageProcessorConfiguration.class);

	@Autowired
	private LabelImageProcessorProperties properties;

	@Bean
	public TensorflowOutputConverter tensorflowOutputConverter() {
		logger.info("Load list: " + properties.getLabelsLocation());
		logger.info("Load LabelImageTensorflowOutputConverter");
		return new LabelImageTensorflowOutputConverter(properties.getLabelsLocation(), properties.getAlternativesLength());
	}

	@Bean
	@RefreshScope
	public TensorflowInputConverter tensorflowInputConverter() throws MalformedURLException {
		logger.info("Load LabelImageTensorflowInputConverter");
		return new LabelImageTensorflowInputConverter();
	}
}
